...
label_dict_val_list = list(label_dict.values())
...
class ScannetDetectionDataset(Dataset):
    ...
    def __init__(self, split_set='train', num_points=20000,
        ...
        ################## TRAIN #################
        # self.data_path = os.path.join(BASE_DIR, 'sl3d_data/SL3D_labels_800_class_ckpt53/')
        ##########################################
        
        ################## EVAL ##################
        self.data_path = os.path.join(BASE_DIR, 'sl3d_data/votenet_scannet_train_detection_data')
        ##########################################
    ...
    def __getitem__(self, idx):
        ...
        ################## TRAINING ##################
        # data = np.load(os.path.join(self.data_path, scan_name)+'.npy')  
        # index_no_wall = np.where(data[:,-1] != 0)
        # data = data[index_no_wall]  
        # index_no_floor = np.where(data[:,-1] != 1)
        # data = data[index_no_floor]  

        # mesh_vertices = data[:,:9]
        # instance_labels = data[:,9]
        # semantic_labels = data[:,10]
        # instance_bboxes = np.load(os.path.join(self.data_path, scan_name)+'_bbox.npy')

       
        # for idx, i in enumerate(semantic_labels):
        #     pos = label_dict_val_list.index(int(i))
        #     semantic_labels[idx] = int(label_dict_key_list[pos])

        # for idx, i in enumerate(instance_bboxes):
        #     pos = label_dict_val_list.index(int(i[-1]))
        #     instance_bboxes[idx][-1] = int(label_dict_key_list[pos])
        ##############################################
        ...
        ################## TRAIN ##################
        # class_ind = class_ind.astype(int)
        ##############################################
