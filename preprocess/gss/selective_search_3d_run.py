...
from features3d import SimilarityMask
from utils import _selective_search_one, post_process

# from dataset.scannet.vis import write_bbox
...
parser.add_argument('--data_path', default='../scannet', 
                        help='path to scannet')
parser.add_argument('--cgal_path', default='../scannet/cgal_output', 
                        help='path to cgal shape detection res')
parser.add_argument('--seg_path', default='../scannet/sl3d_data/scannet_all_points', 
...
def _run_gss(args, scene_id, visualize=False):
    for name, mask in zip(args['names'], args['masks']):
        out_dir = os.path.join('computed_proposal_' + FLAGS.dataset + '_gss', name)
        if FLAGS.dataset == 'scannet':
            os.makedirs(out_dir, exist_ok=True)
            output_file = os.path.join(out_dir, scene_id+'_prop.npy')
            input_file = os.path.join(FLAGS.data_path, 'sl3d_data', FLAGS.dataset+'_all_points', scene_id+'_vert.npy')
        else:
            os.makedirs(os.path.join(out_dir, scene_id), exist_ok=True)
            output_file = os.path.join(out_dir, scene_id, 'prop.npy')
            input_file = os.path.join(FLAGS.data_path, 'sl3d_data', FLAGS.dataset+'_all_points', scene_id, 'vert.npy')
        ...
        if not os.path.isfile(output_file):
            ...
            # use segmentation
            if mask[1] == 1:
                seg_file = os.path.join(FLAGS.seg_path, scene_id+'_sem_label.npy') \
                           if FLAGS.dataset == 'scannet' \
...
if __name__=='__main__':
    ...
    # names = ["size"]
    # masks = [SimilarityMask(2, 0, 0, 0)]

    names = ["size", 'segmentation', 'fill', 'volume']
    masks = [SimilarityMask(1, 0, 0, 0), SimilarityMask(0, 1, 0, 0),
             SimilarityMask(0, 0, 1, 0), SimilarityMask(0, 0, 0, 1)]
