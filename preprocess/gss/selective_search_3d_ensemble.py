...
import numpy as np
import random
from utils import nms_3d_faster

WYPR_DATA_PATH = '../scannet/'
MAX_NUM_PROP = 1000
POLICIES = ["segmentation-size", "segmentation-size"] #  'volume', 'sv', 'sf', 'fv', 'sfv', 'seg_40']
...
if __name__=='__main__':
    ...
    for _ in range(100): #0):
        ...
        for split_set in SPLITS:
            output_dir = 'computed_proposal_scannet_gss_ensamble_test/' + '-'.join(policy)
            ...
            for i in range(len(scan_names)):
                ...
                if os.path.isfile(output_file):
                    continue
                props = [np.load(os.path.join( 'computed_proposal_scannet_gss_ensamble_test/', p, scan_names[i]+'_prop.npy')) for p in policy]
                # props = [np.load(os.path.join('scannet_gss/', scan_names[i] + '_prop.npy'))]
